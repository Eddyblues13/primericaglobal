...
@section('title', 'Primrica Global Capital Dashboard')
        ...
        <div class="heroCard">
            ...
            <div class="balanceBox">
                ...
                <div class="balanceBtns">
                    <a href="{{ route('dashboard.wallet.deposit') }}" class="sbtn deposit-btn"
                        style="text-decoration: none; display: inline-flex; align-items: center; justify-content: center; gap: 6px;"><svg
                            width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                            stroke-width="2.5" stroke-linecap="round" style="flex-shrink: 0;">
                            <path d="M12 5v14M5 12h14" />
                        </svg> Deposit</a>
                    <a href="{{ route('dashboard.wallet.withdraw') }}" class="sbtn ghost withdraw-btn"
                        style="text-decoration: none; display: inline-flex; align-items: center; justify-content: center; gap: 6px;"><svg
                            width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                            stroke-width="2.5" stroke-linecap="round" style="flex-shrink: 0;">
                            <path d="M5 12h14" />
    ...
    <div class="dashboard-charts-grid">
        ...
        <div class="dashboard-chart-card">
            ...
            <div class="chart-value-section">
                ...
                $balanceChangePercent = ($chartData['balance'][0] ?? 0) > 0 ? (($balanceChange /
                ($chartData['balance'][0] ?? 1)) * 100) : 0;
                $isBalancePositive = $balanceChange >= 0;
                @endphp
                <div class="chart-change {{ $isBalancePositive ? 'positive' : 'negative' }}">
                    <span>{{ $isBalancePositive ? '↑' : '↓' }}</span>
                    <span>${{ number_format(abs($balanceChange), 2) }} ({{ $isBalancePositive ? '+' : '' }}{{
                        number_format($balanceChangePercent, 2) }}%)</span>
                </div>
            </div>
            <div class="chart-metrics">
                ...
                <div class="chart-metric">
                    ...
                    <div class="chart-metric-value">${{ number_format(($chartData['balance'][6] ??
                        ($chartData['balance'][count($chartData['balance'] ?? []) - 1] ?? 0)), 2) }}</div>
                </div>
                <div class="chart-metric">
                    <div class="chart-metric-label">Low</div>
                    <div class="chart-metric-value">${{ number_format((!empty($chartData['balance']) ?
                        min($chartData['balance']) : 0), 2) }}</div>
                </div>
                <div class="chart-metric">
                    <div class="chart-metric-label">High</div>
                    <div class="chart-metric-value">${{ number_format((!empty($chartData['balance']) ?
                        max($chartData['balance']) : 0), 2) }}</div>
                </div>
            </div>
        ...
        <div class="dashboard-chart-card">
            ...
            <div class="chart-value-section">
                ...
                $profitChangePercent = ($chartData['profit'][0] ?? 0) > 0 ? (($profitChange / ($chartData['profit'][0]
                ?? 1)) * 100) : 0;
                ...
                <div class="chart-change {{ $isProfitPositive ? 'positive' : 'negative' }}">
                    ...
                    <span>${{ number_format(abs($profitChange), 2) }} ({{ $isProfitPositive ? '+' : '' }}{{
                        number_format($profitChangePercent, 2) }}%)</span>
                </div>
            </div>
            <div class="chart-metrics">
                <div class="chart-metric">
                ...
                <div class="chart-metric">
                    <div class="chart-metric-label">Low</div>
                    <div class="chart-metric-value">${{ number_format((!empty($chartData['profit']) ?
                        min($chartData['profit']) : 0), 2) }}</div>
                </div>
                <div class="chart-metric">
                    <div class="chart-metric-label">High</div>
                    <div class="chart-metric-value">${{ number_format((!empty($chartData['profit']) ?
                        max($chartData['profit']) : 0), 2) }}</div>
                </div>
            </div>
            <div class="chart-period-selectors">
        ...
        <div class="dashboard-chart-card">
            ...
            <div class="chart-value-section">
                ...
                $investmentsChangePercent = ($chartData['investments'][0] ?? 0) > 0 ? (($investmentsChange /
                ($chartData['investments'][0] ?? 1)) * 100) : 0;
                ...
                <div class="chart-change {{ $isInvestmentsPositive ? 'positive' : 'negative' }}">
                    ...
                    <span>${{ number_format(abs($investmentsChange), 2) }} ({{ $isInvestmentsPositive ? '+' : '' }}{{
                        number_format($investmentsChangePercent, 2) }}%)</span>
                </div>
            </div>
            <div class="chart-metrics">
                <div class="chart-metric">
                ...
                <div class="chart-metric">
                    <div class="chart-metric-label">Current</div>
                    <div class="chart-metric-value">${{ number_format(($chartData['investments'][6] ??
                        ($chartData['investments'][count($chartData['investments'] ?? []) - 1] ?? 0)), 2) }}</div>
                </div>
                <div class="chart-metric">
                    <div class="chart-metric-label">Low</div>
                    <div class="chart-metric-value">${{ number_format((!empty($chartData['investments']) ?
                        min($chartData['investments']) : 0), 2) }}</div>
                </div>
                <div class="chart-metric">
                    <div class="chart-metric-label">High</div>
                    <div class="chart-metric-value">${{ number_format((!empty($chartData['investments']) ?
                        max($chartData['investments']) : 0), 2) }}</div>
                </div>
            </div>
            <div class="chart-period-selectors">
    ...
    <div class="lowerGrid">
        ...
        <div class="whitePanel">
            <div class="panelHead">
                <div>
                    <h5>Recent Orders</h5>
                    <small>Your latest Primrica Global Capital purchases</small>
                </div>
            ...
            <div class="orderRow">
                <div class="orderLeft">
                    <div class="thumb">
                        ...
                        <img src="{{ $order->car->image_url }}" alt="{{ $order->car->name }}"
        ...
        <div class="whitePanel">
            ...
            <div class="mItem">
                <div class="mLeft">
                    <div class="logo"
                        style="background:{{ $color['bg'] }};color:{{ $color['text'] }};overflow:hidden;position:relative;padding:0;">
                        <img src="{{ route('stock-logo', ['ticker' => $stock->ticker]) }}" alt="{{ $stock->ticker }}"
