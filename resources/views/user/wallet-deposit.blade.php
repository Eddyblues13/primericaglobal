...
@section('title', 'Primrica Global Capital Wallet - Deposit Funds')
...
<div class="wrap" id="wallet-deposit">
    ...
    <div class="lowerGrid" style="grid-template-columns: 1fr; margin-top: 12px;">
        <div class="walletFormWrap">
            ...
            <form method="POST" action="{{ route('dashboard.wallet.deposit.submit') }}"
                            ...
                            <div style="flex: 1;">
                                ...
                                <div class="methodDetails" data-method-id="{{ $method->id }}">
                                    <div class="methodDetailsText">
                                        ...
                                        for ($i = 0; $i < $lineCount; $i++) { $line=trim($lines[$i]); if (empty($line))
                                            continue; // Check for wallet address (most common patterns) if
                                            (stripos($line, 'address:' ) !==false || stripos($line, 'wallet address:' )
                                            !==false) { $parts=explode(':', $line, 2); $value='' ; // If value is on
                                            same line if (count($parts)> 1 && !empty(trim($parts[1]))) {
                                            $value = trim($parts[1]);
                                            }
                                            // If value is on next line
                                            elseif ($i + 1 < $lineCount && !empty(trim($lines[$i + 1]))) {
                                                $nextLine=trim($lines[$i + 1]); // Check if next line doesn't contain a
                                                colon (not a label) if (stripos($nextLine, ':' )===false) {
                                                $value=$nextLine; $i++; // Skip next line since we used it } } if
                                                (!empty($value)) { $importantParts[]=[ 'label'=> trim($parts[0]),
                                                'value' => $value,
                                                'type' => 'address'
                                                ];
                                                }
                                                ...
                                                // Check for email
                                                elseif (stripos($line, '@') !== false && (stripos($line, 'email') !==
                                                false || stripos($line, 'paypal') !== false)) {
                                                ...
                                                // Check for SWIFT/BIC
                                                elseif (stripos($line, 'SWIFT') !== false || stripos($line, 'BIC') !==
                                                false) {
                                                $parts = explode(':', $line, 2);
                                                if (count($parts) > 1 && !empty(trim($parts[1]))) {
                                                $importantParts[] = [
                                                ...
                                                <div class="detailItem">
                                                    ...
                                                    <div class="detailValue">
                                                        <span class="detailValueText"
                                                            id="detail-{{ $method->id }}-{{ $loop->index }}">{{
                                                            $part['value'] }}</span>
                                                ...
                                                <div class="detailItem">
                                                    <div class="detailValueText"
                                                        style="font-family: inherit; white-space: pre-line;">{{
                                                        $method->details }}</div>
                                                </div>
                                                @endif
                                    </div>
